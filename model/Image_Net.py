...
class ImageRestorer(nn.Module):
    def __init__(self, in_c=1, out_c=1, stage1_width=64, stage2_width=64, num_cab=6):
        ...
        self.tail = nn.Conv2d(stage2_width + stage1_width, out_c, 3, padding=1)

    def forward(self, x_fista):
        stage1_img, encs_feature, decs_feature = self.stage1(x_fista)

        stage1_samfeats, _ = self.sam12(decs_feature[0], x_fista)

        stage2_feat = self.shallow_feat2(x_fista)
        stage2_cat = self.concat12(torch.cat([stage2_feat, stage1_samfeats], dim=1))
        x2_cat = self.orsnet(stage2_cat, encs_feature, decs_feature)

        final_img = self.tail(x2_cat)

        final_out = final_img + x_fista
